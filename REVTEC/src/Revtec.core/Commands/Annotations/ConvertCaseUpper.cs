...
namespace Revtec.core.Commands.Annotations
{
    ...
    public class ConvertCaseUpper : IExternalCommand
    {
            ...
            using (Transaction tx = new Transaction(doc, "Convert Case to Lower"))
            {
                ...
                foreach (var elementId in selectedIds)
                {
                    ...
                    if (elem is TextElement)
                    {
                        ...
                        string result = ConvertCase.ToAllCaps(text);
        ...
        {
            var commandName = typeof(ConvertCaseUpper).Namespace + "." + nameof(ConvertCaseUpper);
            return commandName;
        }
    }
}
