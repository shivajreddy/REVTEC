...
namespace Revtec.core.Commands.Annotations
{
    ...
    public class ConvertCaseLower : IExternalCommand
    {
            ...
            using (Transaction tx = new Transaction(doc, "Convert Case to Lower"))
            {
                ...
                foreach (var elementId in selectedIds)
                {
                    ...
                    if (elem is TextElement)
                    {
                        ...
                        string result = ConvertCase.ToLower(text);
        ...
        {
            var commandName = typeof(ConvertCaseLower).Namespace + "." + nameof(ConvertCaseLower);
            return commandName;
        }
